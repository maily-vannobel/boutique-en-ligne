document.addEventListener("DOMContentLoaded", function () {
    function addImageUrlField() {
        ...
        input.className =
            "w-full px-5 py-1 text-gray-700 bg-gray-200 rounded mt-2";
    ...
    function addFilter(filterValue, filterId, filterType) {
        if (
            !document.querySelector(
                `#selected-filters .filter[data-id='${filterId}']`
            )
        ) {
            let filterDiv = document.createElement("div");
            filterDiv.className =
                "filter bg-gray-200 rounded px-2 py-1 m-1 flex items-center";
            filterDiv.setAttribute("data-id", filterId);
            filterDiv.innerHTML = `
                <span>${filterType}: ${filterValue}</span>
                <button type="button" class="ml-2 text-red-500 remove-filter">✖</button>
                <input type="hidden" name="filters[]" value="${filterId}">
            `;
    ...
    // Load initially selected filters
    selectedFilters.forEach(function (filter) {
        addFilter(filter.value, filter.id, filter.type);
    });

    // Load initially selected subcategories
    selectedSubcategories.forEach(function (subcategory) {
        addSubcategory(subcategory.name, subcategory.id);
    });

    // Add initial image URL field if none exist
    if (document.querySelectorAll("input[name='image_urls[]']").length === 0) {
        addImageUrlField();
    }

    document
        .getElementById("addImageUrlButton")
        .addEventListener("click", addImageUrlField);
    document
        .getElementById("removeImageUrlButton")
        .addEventListener("click", removeImageUrlField);
});
